...
<!--          <el-button type="success" @click="dialogFormVisibles()" round>领养</el-button>-->
        </div>
      </div>
      ...
      <div class="blog-user">
        ...
        <div class="block" v-if="blog.user" style="display:flex;">
          <el-avatar :size="50" shape="square" :src="blog.user.avatar"></el-avatar>
          <div style="text-align: initial;margin-left: 10px;">
            <div v-if="blog.user" class="blog-user-name">
              <span>{{blog.user.name}}</span>
            </div>
            <div class="blog-user-time">时间: {{blog.dateTime}} <i class="el-icon-time"/></div>
          </div>
        </div>
        <!--浏览信息-->
<!--        <div class="blog-user-icon">
          ...
          <div style="margin-left: 10px;"><i class="el-icon-thumb" style="margin-left: 5px;"></i><span style="margin-left: 5px;">0</span></div>
        </div>-->
<!--        <div>
          <el-button type="success" @click="addLike(petBlog.id, userId)" round>点赞</el-button>
        </div>-->
      </div>
      <!--宠物内容-->
      <div>
        <div>
          <el-divider content-position="left"><h3>博客封面</h3></el-divider>
        ...
        <div style="margin-top: 30px">
          ...
          <div style="text-align: start;margin-left: 20px; white-space: pre-line;">
            <p>{{blog.introduce}}</p>
          </div>
        </div>
...
export default {
  ...
  created () {
    ...
    this.$axios.get('http://localhost:8081/petBlog/queryById/' + _this.$route.query.id).then(function (resp) {
